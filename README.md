...
- **Humans** shows people's comments and reviews. **Bots** shows everything posted by apps, CI, and review
  bots. **All** is GitHub's normal view.
...
| Timeline events from people | Both lanes | Where labels, commits, reviews and merges show up. Events from bots always follow the Bots lane |
| Remember the lane per pull request | off | Keep a per-PR choice instead of one global lane |
| Always show the first comment | on | Keeps the PR description visible even when a bot opened the PR |
...
In order: the human override list, a `name[bot]` login, the built-in bot list (about 45 common apps —
`github-actions`, `codecov`, `coderabbitai`, `greptile`, `dependabot`, `sonarcloud`, …), a GitHub App avatar
(`avatars.githubusercontent.com/in/…`), the `bot` badge in the comment header, then the name heuristics.
...
test/
  ...
  fixture.html           Open in a browser to eyeball classification and the lane CSS
```
...
- Classification reads GitHub's DOM. The classic timeline (`.js-discussion`, `.TimelineItem`, including the
  ...
  each child of the timeline container as one item. If GitHub reshuffles its markup, `content/classify.js`
